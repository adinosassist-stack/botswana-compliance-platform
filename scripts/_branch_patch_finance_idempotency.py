...
p=Path('cloudflare/src/finance-core.js')
s=p.read_text()

anchor='''function sourceFingerprintBasis({tenantId,accountId,sourceType,provider,idempotencyKey,row,index}){
...
'''
if anchor not in s: raise SystemExit('missing source fingerprint anchor')
s=s.replace(anchor,addition,1)

connection_anchor='''async function recordAdapterSyncCompletion({env,auth,connection,idempotencyKey,batchId,rowCount,importedCount,duplicateCount,id,sha256Hex,writeAudit}){
...
'''
if connection_anchor not in s: raise SystemExit('missing connection sync anchor')
s=s.replace(connection_anchor,connection_anchor+verifier,1)

old='''    const existing=await env.DB.prepare("SELECT id,row_count,imported_count,duplicate_count,status FROM finance_import_batches WHERE tenant_id=? AND idempotency_key=? LIMIT 1").bind(auth.tenant_id,idempotencyKey).first();
    if(existing){
...
'''
new='''    const normalized=[];for(let i=0;i<rows.length;i++){const item=normalizeFinanceRow(rows[i],i);if(!item.ok)return json({error:item.error,row:i},400);normalized.push(item.row)}
    ...
    const packedJson=JSON.stringify(packed);
'''
if old not in s: raise SystemExit('missing finance import replay block')
s=s.replace(old,new,1)

old_race='''        const replay=await env.DB.prepare("SELECT id,row_count,imported_count,duplicate_count,status FROM finance_import_batches WHERE tenant_id=? AND idempotency_key=? LIMIT 1").bind(auth.tenant_id,idempotencyKey).first();
        if(replay){
...
'''
new_race='''        const replay=await env.DB.prepare("SELECT id,account_id,source_type,provider,row_count,imported_count,duplicate_count,status FROM finance_import_batches WHERE tenant_id=? AND idempotency_key=? LIMIT 1").bind(auth.tenant_id,idempotencyKey).first();
        if(replay){
...
'''
if old_race not in s: raise SystemExit('missing finance race replay block')
s=s.replace(old_race,new_race,1)

old_export='export const __financeTest=Object.freeze({normalizeFinanceRow,validDate,sourceFingerprintBasis,ACCOUNT_TYPES,SOURCE_TYPES,MAX_IMPORT_ROWS});'
new_export='export const __financeTest=Object.freeze({normalizeFinanceRow,validDate,sourceFingerprintBasis,sameFingerprintSet,ACCOUNT_TYPES,SOURCE_TYPES,MAX_IMPORT_ROWS});'
if old_export not in s: raise SystemExit('missing finance test export')
s=s.replace(old_export,new_export,1)
p.write_text(s)

r=Path('.github/workflows/recovery-ci.yml')
rs=r.read_text()
ci_anchor='          node tests/v79-finance-reconciliation.mjs\n'
if ci_anchor not in rs: raise SystemExit('missing Recovery CI finance anchor')
if 'v82-finance-idempotency-replay-integrity.mjs' not in rs:
    rs=rs.replace(ci_anchor,ci_anchor+'          node tests/v82-finance-idempotency-replay-integrity.mjs\n',1)
r.write_text(rs)
