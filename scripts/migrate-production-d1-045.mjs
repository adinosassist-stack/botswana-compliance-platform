...
const migrationPath = 'cloudflare/migrations/045_v80_agentic_foundation.sql';
const expectedGitBlobSha = '6485a530339f4b3233d0725d10d5ccbefde1e039';
const targetTables = ['agentic_runs','agentic_proposals','agentic_events'];
const targetIndexes = ['agentic_runs_tenant_idx','agentic_proposals_tenant_idx','agentic_proposals_run_idx','agentic_events_tenant_idx'];

function fail(message) {
  throw new Error(`Production D1 migration 045 refused: ${message}`);
}
...
if (targetComplete(before)) {
  console.log('Production D1 migration 045 already present; no mutation required.');
  process.exit(0);
}
...
console.log('Applying reviewed forward-only migration 045 to production D1.');
...
console.log('Production D1 migration 045 verified successfully.');
