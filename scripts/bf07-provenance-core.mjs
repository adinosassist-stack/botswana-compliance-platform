...
export function validateProvenance(root,env=process.env){
  ...
  if(p.provider==='github-actions'){
    if(!fullSha(p.source?.commit)||!p.source?.repository||!p.source?.ref||!p.source?.runId||!p.source?.runAttempt)throw new Error('GitHub BF-07 provenance context is incomplete');
  }
  if(env.GITHUB_ACTIONS==='true'){
    const current=githubSource(env);
    if(!fullSha(current.commit)||!current.repository||!current.ref||!current.runId||!current.runAttempt)throw new Error('Current GitHub BF-07 execution context is incomplete');
