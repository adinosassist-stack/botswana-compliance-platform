...
let {r:get,d:gd}=await jsonFetch('/api/state',{headers:{Cookie:cookie}});if(get.status!==200||!Number.isInteger(gd.version)||gd.version<1)throw new Error(`state get failed numeric version contract ${JSON.stringify(gd)}`);
const state={activeCompanyId:'co_test',activeRole:'owner',companies:[{id:'co_test',profile:{name:'Test'}}],audit:[]};
let {r:put,d:pd}=await jsonFetch('/api/state',{method:'PUT',headers,body:JSON.stringify({state,version:gd.version})});if(put.status!==200||!Number.isInteger(pd.version)||pd.version<=gd.version)throw new Error(`state update/version failed ${put.status} ${JSON.stringify(pd)}`);
...
let {r:get2,d:gd2}=await jsonFetch('/api/state',{headers:{Cookie:cookie2}});if(get2.status!==200||!Number.isInteger(gd2.version))throw new Error('second tenant state failed numeric version contract');
