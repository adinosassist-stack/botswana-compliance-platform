...
create table if not exists app_state(tenant_id uuid primary key references tenants(id) on delete cascade,state jsonb not null default '{}'::jsonb,updated_at timestamptz not null default now(),version bigint not null default 1);
