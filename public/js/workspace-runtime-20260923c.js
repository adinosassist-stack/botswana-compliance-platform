...
function renderOpsLocations(items){const box=document.getElementById("opsLocationsList");if(!box)return;globalThis.__opsLocationsById=Object.fromEntries((items||[]).map(x=>[String(x.id),x]));box.safeHTML=(items||[]).map(x=>{const active=Number(x.active)===1;return `<div class="item"><div class="between row"><div><b>${escapeHtml(x.name)}</b><div class="muted small">${escapeHtml(x.code||"No code")}${x.town?` · ${escapeHtml(x.town)}`:""} · ${active?"Active":"Inactive"}</div></div><div class="actions"><button class="btn alt" data-bw-onclick="editOpsLocation('${escapeHtml(x.id)}')">Edit</button><button class="btn alt" data-bw-onclick="${active?`deactivateOpsLocation('${escapeHtml(x.id)}')`:`reactivateOpsLocation('${escapeHtml(x.id)}')`}">${active?"Deactivate":"Reactivate"}</button></div></div></div>`}).join("")||'<div class="muted small">No locations configured.</div>'}
...
async function addOpsLocation(){const name=document.getElementById("opsLocationName").value.trim();if(!name)return notifyUser("Enter a location name.");try{const saved=await apiJson("/api/daily-reporting/locations",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name,code:document.getElementById("opsLocationCode").value,town:document.getElementById("opsLocationTown").value})});document.getElementById("opsLocationName").value="";document.getElementById("opsLocationCode").value="";document.getElementById("opsLocationTown").value="";await renderDailyOperations();notifyUser(saved?.reusedDefault?"Location saved. The initial Head Office placeholder was replaced.":"Location added.",{type:"success"})}catch(e){notifyUser(e.message)}}
...
async function deactivateOpsLocation(id){const current=globalThis.__opsLocationsById?.[String(id)],confirmed=await BW.dialog.confirm({title:"Deactivate location?",message:`Deactivate ${current?.name||"this location"}? Historical reports stay intact and active employee reporting links for this location will be revoked.`,confirmLabel:"Deactivate",danger:true});if(!confirmed)return;try{await apiJson(`/api/daily-reporting/locations/${encodeURIComponent(id)}/deactivate`,{method:"POST",body:"{}"});await renderDailyOperations();await renderPeopleReportingSetup();notifyUser("Location deactivated. Reporting links for it were revoked.",{type:"success"})}catch(e){notifyUser(e.message)}}
