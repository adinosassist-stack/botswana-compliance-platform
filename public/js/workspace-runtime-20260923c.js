...
async function addEmployeeRecord(){
  ...
  try{
    ...
    if(document.getElementById("opsReporterEmployee"))await renderDailyOperations();
...
async function renderEmployeeRegister(){
  ...
  try{
    const response=await apiJson("/api/employees"),arr=response.items||[],active=arr.filter(e=>String(e.status||"").trim().toLowerCase()==="active"),contracts=active.filter(e=>Number(e.contract_signed||0)===1).length,assets=active.filter(e=>Number(e.asset_acknowledgement||0)===1).length,total=active.length;
    if(by("empRecordsCount"))by("empRecordsCount").textContent=total;if(by("contractCoverage"))by("contractCoverage").textContent=(total?Math.round(contracts/total*100):0)+"%";if(by("assetCoverage"))by("assetCoverage").textContent=(total?Math.round(assets/total*100):0)+"%";
    el.safeHTML=arr.length?arr.map(e=>{const active=String(e.status||"").trim().toLowerCase()==="active";return `<div class="item"><div class="between row"><div><b>${escapeHtml(e.full_name)}</b><div class="muted small">${escapeHtml(e.role_title||"Role not set")} · ${e.start_date||"Start date not set"} · ${active?"active":"inactive"}</div></div><div class="actions"><span class="badge ${active?"good":""}">${active?"Active":"Removed"}</span>${active?`<button class="btn alt" type="button" data-bw-onclick="removeEmployeeRecord('${safeId(e.id)}')">Remove</button>`:""}</div></div><div class="rulemeta"><span class="pill">Contract ${Number(e.contract_signed||0)===1?"✓":"missing"}</span><span class="pill">Asset form ${Number(e.asset_acknowledgement||0)===1?"✓":"missing"}</span></div></div>`}).join(""):`<div class="muted small">No employee records yet. Add one here and it will also become available under Employee reporting access.</div>`;
...
function renderOpsLocations(items){const box=document.getElementById("opsLocationsList");if(!box)return;const active=(items||[]).filter(x=>Number(x.active)!==0);box.safeHTML=active.length?active.map(x=>`<div class="item"><div class="between row"><div><b>${escapeHtml(x.name)}</b><div class="muted small">${escapeHtml(x.code||"No code")}${x.town?` · ${escapeHtml(x.town)}`:""}</div></div><button class="btn alt" type="button" data-bw-onclick="removeOpsLocation('${safeId(x.id)}')">Remove</button></div></div>`).join(""):'<div class="muted small">No active locations configured.</div>'}
...
async function renderPeopleReportingSetup(){
  ...
  const locs=(locations.items||[]).filter(x=>Number(x.active)!==0),emps=(employees.items||[]).filter(x=>String(x.status||"").trim().toLowerCase()==="active");
  ...
  if(locationsR.status==="fulfilled")renderOpsLocations(locs);
...
async function renderDailyOperations(){
  ...
  const requests=[
    ...
    apiJson(`/api/daily-reporting/performance?date=${encodeURIComponent(date)}${locationId?`&locationId=${encodeURIComponent(locationId)}`:""}`)
  ];
  const [locationsR,employeesR,accessR,settingsR,dashboardR,summariesR,performanceR]=await Promise.allSettled(requests);
  const value=(result,fallback)=>result.status==="fulfilled"?result.value:fallback;
  const reason=result=>result.status==="rejected"?result.reason:null;
  const locations=value(locationsR,{items:[]}),employees=value(employeesR,{items:[]}),access=value(accessR,{items:[]});
  const settings=value(settingsR,{}),dashboard=value(dashboardR,{}),summaries=value(summariesR,{items:[]}),performance=value(performanceR,null);
  const locs=(locations.items||[]).filter(x=>Number(x.active)!==0),emps=(employees.items||[]).filter(x=>String(x.status||"").trim().toLowerCase()==="active");
  ...
  if(performanceR.status==="fulfilled")renderOpsPerformance(performance);
  const reportingFailures=[settingsR,dashboardR,summariesR,performanceR].filter(x=>x.status==="rejected");
...
async function refreshOpsManagementSurface(){if(document.getElementById("peopleops")?.classList.contains("active"))await renderPeopleReportingSetup();if(document.getElementById("dailyreports")?.classList.contains("active"))await renderDailyOperations()}
async function addOpsLocation(){const name=document.getElementById("opsLocationName").value.trim();if(!name)return notifyUser("Enter a location name.");try{const saved=await apiJson("/api/daily-reporting/locations",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name,code:document.getElementById("opsLocationCode").value,town:document.getElementById("opsLocationTown").value})});document.getElementById("opsLocationName").value="";document.getElementById("opsLocationCode").value="";document.getElementById("opsLocationTown").value="";await refreshOpsManagementSurface();notifyUser(saved?.reusedDefault?"Location saved. The initial Head Office placeholder was replaced.":saved?.reactivated?"Location restored.":"Location added.",{type:"success"})}catch(e){notifyUser(e.message)}}
async function removeOpsLocation(id){if(STANDALONE_PREVIEW)return notifyUser("Location removal is available in the authenticated workspace.");const confirmed=await BW.dialog.confirm({title:"Remove location?",message:"Remove this location from active use? Historical reports will be retained and any active employee reporting links for this location will be revoked.",confirmLabel:"Remove location",danger:true});if(!confirmed)return;try{await apiJson(`/api/daily-reporting/locations/${encodeURIComponent(id)}`,{method:"DELETE"});await refreshOpsManagementSurface();notifyUser("Location removed from active use. Historical reports were retained and reporting links for this location were revoked.",{type:"success"})}catch(e){notifyUser(e.message)}}
