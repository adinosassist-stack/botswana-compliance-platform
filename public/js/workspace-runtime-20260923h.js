...
const reportingAnalyticsApiClient=BW.api.createClient({getCsrfToken:()=>csrfToken,onUnauthorized:()=>{try{showAuth()}catch{}},onError:(err)=>console.warn("BW reporting analytics request failed",{code:err.code,status:err.status}),timeoutMs:30000,retries:0,candidateTimeoutMs:9000});
...
async function reportingAnalyticsJson(url,opts={}){return STANDALONE_PREVIEW?runtimeApiClient.request(url,opts):reportingAnalyticsApiClient.request(url,opts)}
...
async function renderEmployeesForHr(){
  ...
  try{
    ...
    const items=(r.items||[]).filter(x=>String(x.status||"").trim().toLowerCase()==="active");
