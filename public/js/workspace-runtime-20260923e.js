...
async function openEmployeeReportingAccess(id){
 ...
 try{
   ...
   const accessHtml=activeAccess.length?activeAccess.map(x=>`<div class="item" style="margin-top:7px"><b>${escapeHtml(x.location_name||"Location")}</b><div class="muted small">Active until ${new Date(x.expires_at).toLocaleDateString()}${x.last_used_at?` · last used ${new Date(x.last_used_at).toLocaleDateString()}`:" · not used yet"}</div></div>`).join(""):'<div class="muted small">No active reporting access yet.</div>';
   const options=locations.map(x=>`<option value="${escapeHtml(x.id)}">${escapeHtml(x.name)}</option>`).join("");
   box.safeHTML=`<div class="notice info"><b>Employee reporting access</b><div class="small">Existing private bearer links are stored only as hashes and cannot be revealed again. Create or rotate a link below to show a fresh private link.</div></div>${accessHtml}${locations.length?`<div class="formgrid" style="margin-top:9px"><div><label for="employeeReportingLocation_${safeId(id)}">Location</label><select id="employeeReportingLocation_${safeId(id)}">${options}</select></div></div><button class="btn soft" type="button" style="margin-top:8px" data-bw-onclick="createEmployeeReportingLinkFromCard('${safeId(id)}')">Create / rotate and show link</button><div id="employeeReportingLinkResult_${safeId(id)}"></div>`:'<div class="muted small" style="margin-top:8px">Add an active location before issuing reporting access.</div>'}`;
