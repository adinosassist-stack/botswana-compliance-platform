...
async function renderEmployeeRegister(){
  ...
  try{
    ...
    if(by("empRecordsCount"))by("empRecordsCount").textContent=total;if(by("contractCoverage"))by("contractCoverage").textContent=(total?Math.round(contracts/total*100):0)+"%";if(by("assetCoverage"))by("assetCoverage").textContent=(total?Math.round(assets/total*100):0)+"%";
    el.safeHTML=arr.length?arr.map(e=>`<div class="item"><div class="between row"><div><b>${escapeHtml(e.full_name)}</b><div class="muted small">${escapeHtml(e.role_title||"Role not set")} · ${e.start_date||"Start date not set"} · ${escapeHtml(String(e.status||"active"))}</div></div><span class="badge ${String(e.status||"").toLowerCase()==="active"?"good":""}">${String(e.status||"active").toLowerCase()==="active"?"Active":"Inactive"}</span></div><div class="rulemeta"><span class="pill">Contract ${Number(e.contract_signed||0)===1?"✓":"missing"}</span><span class="pill">Asset form ${Number(e.asset_acknowledgement||0)===1?"✓":"missing"}</span></div></div>`).join(""):`<div class="muted small">No employee records yet. Add one here and it will also become available under Employee reporting access.</div>`;
...
async function renderDailyOperations(){
  ...
  const locs=locations.items||[],emps=(employees.items||[]).filter(x=>String(x.status||"").trim().toLowerCase()==="active");
