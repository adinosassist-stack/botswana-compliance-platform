...
const COMMAND_HUBS=new Set(["dashboard","moneyhub","workhub","peopleops","protecthub","businesshub","evidencehub","tenderhub","automationhub","accounthub"]);
const COMMAND_GROUP_ORDER=["Money","Work","People","Protect","Business","Documents","Tenders","AI","Settings","Specialist","Administration"];
...
async function renderMoneyHub(){
 if(!roleCanView("moneyhub")||!["owner","manager"].includes(currentWorkspaceRole()))return;
 const set=(id,value)=>{const el=document.getElementById(id);if(el)el.textContent=value};
 try{const f=await apiJson("/api/finance/summary"),receivables=f.receivables||{},accounts=Array.isArray(f.accounts)?f.accounts:[],recon=f.reconciliation||{};set("moneyCashPosition",homeMoneyMinor(f.cashPositionMinor));set("moneyReceivables",homeMoneyMinor(receivables.outstandingMinor));set("moneyReconIssues",Number(recon.unresolvedCount||0));set("moneyCashDetail",`Canonical Finance Core · ${accounts.length} active account${accounts.length===1?"":"s"} · ${Number(recon.unresolvedCount||0)} unresolved reconciliation item${Number(recon.unresolvedCount||0)===1?"":"s"}.`);set("moneyReceivablesDetail",`${Number(receivables.outstandingInvoiceCount||0)} outstanding invoice${Number(receivables.outstandingInvoiceCount||0)===1?"":"s"} · ${Number(receivables.overdueInvoiceCount||0)} overdue.`)}
 catch(e){set("moneyCashPosition","Unavailable");set("moneyReceivables","Unavailable");set("moneyReconIssues","—");set("moneyCashDetail","Finance Core could not be confirmed. Do not interpret unavailable as a zero balance.");set("moneyReceivablesDetail","Receivables could not be confirmed from the canonical invoice ledger.")}
}
function propertyNumber(id,{min=0,max=Number.POSITIVE_INFINITY,blank=0}={}){const el=document.getElementById(id),raw=String(el?.value??"").trim();if(!raw)return blank;const n=Number(raw);if(!Number.isFinite(n))return NaN;return Math.min(max,Math.max(min,n))}
function propertyMoney(n){const value=Number(n);if(!Number.isFinite(value))return "—";return "P"+value.toLocaleString("en-BW",{minimumFractionDigits:0,maximumFractionDigits:0})}
function propertyPercent(n,digits=1){const value=Number(n);if(!Number.isFinite(value))return "—";return value.toLocaleString("en-BW",{minimumFractionDigits:digits,maximumFractionDigits:digits})+"%"}
function propertyLoanBalance(principal,monthlyRate,totalPayments,paymentsMade,payment){if(principal<=0)return 0;const n=Math.max(0,Math.min(totalPayments,paymentsMade));if(monthlyRate===0)return Math.max(0,principal-payment*n);const factor=Math.pow(1+monthlyRate,n);return Math.max(0,principal*factor-payment*((factor-1)/monthlyRate))}
function propertyDealScenario(){const purchase=propertyNumber("propertyPurchasePrice"),rent=propertyNumber("propertyMonthlyRent"),vacancy=propertyNumber("propertyVacancy",{min:0,max:100}),operating=propertyNumber("propertyOperatingCosts"),deposit=propertyNumber("propertyDeposit"),acquisition=propertyNumber("propertyAcquisitionCosts"),rate=propertyNumber("propertyInterestRate",{min:0,max:100}),years=propertyNumber("propertyLoanYears",{min:1,max:50,blank:20}),appreciation=propertyNumber("propertyAppreciation",{min:-100,max:100});if(!Number.isFinite(purchase)||!Number.isFinite(rent)||purchase<=0||rent<=0)return null;const effectiveDeposit=Math.min(purchase,Math.max(0,deposit)),loan=Math.max(0,purchase-effectiveDeposit),annualScheduledRent=rent*12,effectiveRent=annualScheduledRent*(1-vacancy/100),noi=effectiveRent-operating,grossYield=annualScheduledRent/purchase*100,netYield=noi/purchase*100,monthlyRate=rate/100/12,totalPayments=Math.max(1,Math.round(years*12)),monthlyDebt=loan<=0?0:(monthlyRate===0?loan/totalPayments:loan*monthlyRate/(1-Math.pow(1+monthlyRate,-totalPayments))),annualDebt=monthlyDebt*12,cashFlow=noi-annualDebt,cashInvested=effectiveDeposit+acquisition,coc=cashInvested>0?cashFlow/cashInvested*100:NaN,dscr=annualDebt>0?noi/annualDebt:NaN,breakEven=annualScheduledRent>0?(operating+annualDebt)/annualScheduledRent*100:NaN,value5=purchase*Math.pow(1+appreciation/100,5),balance5=propertyLoanBalance(loan,monthlyRate,totalPayments,60,monthlyDebt),equity5=value5-balance5;return {purchase,rent,vacancy,operating,deposit:effectiveDeposit,acquisition,rate,years,appreciation,loan,annualScheduledRent,effectiveRent,noi,grossYield,netYield,monthlyDebt,annualDebt,cashFlow,cashInvested,coc,dscr,breakEven,value5,balance5,equity5,name:String(document.getElementById("propertyDealName")?.value||"").trim().slice(0,120)}}
function propertyRiskNotes(x){const notes=[];if(x.noi<=0)notes.push(["warn","Operating assumptions produce zero or negative NOI before debt service."]);if(x.annualDebt>0&&x.dscr<1)notes.push(["warn","NOI does not cover modeled annual debt service (DSCR below 1.0×)."]);else if(x.annualDebt>0&&x.dscr<1.2)notes.push(["warn","Debt-service coverage is thin in this scenario (DSCR below 1.20×)."]);if(x.cashFlow<0)notes.push(["warn","Modeled annual cash flow is negative after operating costs and debt service."]);if(Number.isFinite(x.breakEven)&&x.breakEven>90)notes.push(["warn","Break-even occupancy is above 90%, leaving little room for vacancy or collection slippage."]);if(x.vacancy===0)notes.push(["warn","Vacancy is set to 0%. Stress-test a non-zero vacancy assumption before relying on the result."]);if(x.operating===0)notes.push(["warn","Annual operating costs are set to P0. Add rates, insurance, maintenance, management and other recurring costs where applicable."]);if(x.loan>0&&x.rate===0)notes.push(["warn","Loan interest is set to 0%. Enter the actual lender rate or the modeled debt service will be understated."]);if(x.acquisition===0)notes.push(["warn","Acquisition costs are set to P0. Add transfer, legal, valuation and loan costs where applicable."]);if(!notes.length)notes.push(["good","No basic cash-flow warning threshold was triggered by the assumptions entered. This is not an all-clear on value, title, condition, financing or legal risk."]);return notes}
function calculatePropertyDeal(){const x=propertyDealScenario(),box=document.getElementById("propertyDealResults");if(!box)return false;if(!x){box.safeHTML='<div class="notice bad"><b>Add a valid purchase price and monthly rent.</b><div class="small">Both must be greater than zero before Thebe can calculate the scenario.</div></div>';return false}const debtLabel=x.annualDebt>0?(Number.isFinite(x.dscr)?x.dscr.toLocaleString("en-BW",{minimumFractionDigits:2,maximumFractionDigits:2})+"×":"—"):"No debt",cocLabel=Number.isFinite(x.coc)?propertyPercent(x.coc):"No cash base",notes=propertyRiskNotes(x);box.safeHTML=`<div class="property-result-grid"><div class="property-metric"><span>Gross rental yield</span><b>${propertyPercent(x.grossYield)}</b><small>Scheduled annual rent ÷ purchase price.</small></div><div class="property-metric"><span>Net yield</span><b>${propertyPercent(x.netYield)}</b><small>NOI ÷ purchase price after vacancy and operating costs.</small></div><div class="property-metric"><span>Net operating income</span><b>${propertyMoney(x.noi)}</b><small>Effective annual rent less annual operating costs.</small></div><div class="property-metric"><span>Monthly debt service</span><b>${propertyMoney(x.monthlyDebt)}</b><small>Standard amortising payment from the rate and term entered.</small></div><div class="property-metric"><span>Annual cash flow</span><b>${propertyMoney(x.cashFlow)}</b><small>NOI less modeled annual debt service.</small></div><div class="property-metric"><span>DSCR</span><b>${debtLabel}</b><small>NOI ÷ annual debt service; shown only when debt exists.</small></div><div class="property-metric"><span>Cash-on-cash return</span><b>${cocLabel}</b><small>Annual cash flow ÷ deposit and acquisition costs entered.</small></div><div class="property-metric"><span>Break-even occupancy</span><b>${propertyPercent(x.breakEven)}</b><small>Occupancy needed to cover operating costs and modeled debt service.</small></div></div><div class="property-summary"><div class="between row"><div><div class="section-eyebrow">Five-year scenario</div><h3>${escapeHtml(x.name||"Property scenario")}</h3></div><span class="badge">Assumption-based</span></div><div class="property-summary-line"><span class="muted">Modeled property value in year 5</span><b>${propertyMoney(x.value5)}</b></div><div class="property-summary-line"><span class="muted">Estimated loan balance after 5 years</span><b>${propertyMoney(x.balance5)}</b></div><div class="property-summary-line"><span class="muted">Illustrative year-5 equity</span><b>${propertyMoney(x.equity5)}</b></div><div class="muted small" style="margin-top:10px">Year-5 value is generated only from your appreciation assumption (${propertyPercent(x.appreciation)} per year). It is not a forecast or valuation.</div><div class="property-risk-list">${notes.map(n=>`<div class="property-risk-item ${n[0]}">${escapeHtml(n[1])}</div>`).join("")}</div><div class="property-agent-actions"><button class="btn" type="button" data-bw-onclick="askThebeAboutProperty()">Discuss this deal with Thebe</button><button class="btn alt" type="button" data-bw-onclick="showView('moneyhub')">Back to Money</button></div></div>`;return true}
function resetPropertyDeal(){["propertyDealName","propertyPurchasePrice","propertyMonthlyRent","propertyVacancy","propertyOperatingCosts","propertyDeposit","propertyAcquisitionCosts","propertyInterestRate","propertyAppreciation"].forEach(id=>{const el=document.getElementById(id);if(el)el.value=""});const years=document.getElementById("propertyLoanYears");if(years)years.value="20";const box=document.getElementById("propertyDealResults");if(box)box.safeHTML='<div class="card property-empty"><div class="section-eyebrow">Deal analysis</div><h3>Enter a purchase price and monthly rent.</h3><p class="muted small">Thebe will show gross yield, net operating income, net yield, debt service, DSCR, annual cash flow, cash-on-cash return, break-even occupancy and an illustrative five-year equity scenario.</p></div>'}
function askThebeAboutProperty(){const x=propertyDealScenario();if(!x){calculatePropertyDeal();return false}const prompt=`Analyse this property scenario using only these supplied assumptions. Do not invent market value or comparable-sales data. Property: ${x.name||"unnamed"}. Purchase price ${propertyMoney(x.purchase)}; monthly rent ${propertyMoney(x.rent)}; vacancy ${propertyPercent(x.vacancy)}; annual operating costs ${propertyMoney(x.operating)}; deposit ${propertyMoney(x.deposit)}; acquisition costs ${propertyMoney(x.acquisition)}; loan ${propertyMoney(x.loan)} at ${propertyPercent(x.rate,2)} for ${x.years} years; appreciation assumption ${propertyPercent(x.appreciation)}. Calculated NOI ${propertyMoney(x.noi)}, net yield ${propertyPercent(x.netYield)}, annual cash flow ${propertyMoney(x.cashFlow)}, DSCR ${x.annualDebt>0?x.dscr.toFixed(2)+"x":"no debt"}, cash-on-cash ${Number.isFinite(x.coc)?propertyPercent(x.coc):"not available"}, break-even occupancy ${propertyPercent(x.breakEven)}. Explain the drivers, stress-test the assumptions conceptually, and list due-diligence questions. Treat this as scenario analysis, not a valuation or investment recommendation.`;return openThebeFromHome(prompt,false)}
