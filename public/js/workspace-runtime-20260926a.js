...
function calculatePropertyDeal(){
  const x=propertyDealScenario(),box=document.getElementById("propertyDealResults");if(!box)return false;if(!x){box.safeHTML='<div class="notice bad"><b>Add a valid purchase price and monthly rent.</b><div class="small">Both must be greater than zero before Thebe can calculate the scenario.</div></div>';return false}
  const debtLabel=x.annualDebt>0?(Number.isFinite(x.dscr)?x.dscr.toLocaleString("en-BW",{minimumFractionDigits:2,maximumFractionDigits:2})+"×":"—"):"No debt",cocLabel=Number.isFinite(x.coc)?propertyPercent(x.coc):"No cash base",notes=propertyRiskNotes(x);
