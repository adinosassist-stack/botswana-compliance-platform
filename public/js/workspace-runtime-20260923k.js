...
const reportingAnalyticsApiClient=BW.api.createClient({getCsrfToken:()=>csrfToken,onUnauthorized:()=>{try{showAuth()}catch{}},onError:(err)=>console.warn("BW reporting analytics request failed",{code:err.code,status:err.status}),timeoutMs:60000,retries:1,candidateTimeoutMs:20000});
...
function scheduleOwnerOnboarding(){
 ...
 window.__THEBE_ONBOARDING_AUTO_PROMPT_SETTLED__=false;
 if(!eligible()){window.__THEBE_ONBOARDING_AUTO_PROMPT_SETTLED__=true;return false}
 const settle=()=>{window.__THEBE_ONBOARDING_AUTO_PROMPT_SETTLED__=true};
 const afterReady=()=>{
  requestAnimationFrame(()=>requestAnimationFrame(()=>queueMicrotask(()=>{
   if(!eligible()||window.__THEBE_WORKSPACE_READY__!==true){settle();return}
   if(document.querySelector(".modal.open")&&!document.getElementById("onboardModal")?.classList.contains("open")){settle();return}
   openOnboarding();settle();
  })));
 };
