...
ok(workspaceHtml.includes('async function hydrateWorkspaceAudit()')&&workspaceHtml.includes('nextStore.activeRole=currentUser.role;nextStore.audit=[];replaceWorkspaceStore(nextStore);')&&workspaceHtml.includes('logEvent("APP_OPENED",{ruleset:"BW-2026.08.30-launch"});void hydrateWorkspaceAudit();const landing='), 'audit hydration is explicitly deferred until after authoritative store render and workspace reveal');
...
console.log(`Phase 0 deferred integration, post-deploy, tenant-integrity and residue-audit contract: ${pass}/38 PASS`);
