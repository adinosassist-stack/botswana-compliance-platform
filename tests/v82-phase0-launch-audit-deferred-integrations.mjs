...
ok(audit.includes("body.matchAll(/\\b([A-Za-z0-9_]*tenant_id)\\b/gi)")&&audit.includes('D1_COMPOUND_SELECT_BATCH_SIZE=20')&&audit.includes('dependencySelects.slice(offset,offset+D1_COMPOUND_SELECT_BATCH_SIZE)')&&audit.includes("batch.join(' UNION ALL ')"), 'tenant integrity audit derives a complete tenant-reference dependency sweep in bounded D1-safe batches');
