...
assert.match(runtime,/const reportingSetupDetails=document\.getElementById\("opsReportingSetupDetails"\);/);
assert.match(runtime,/if\(reportingSetupDetails&&!reportingSetupDetails\.open\)reportingSetupDetails\.open=true;/,"People reporting setup must remain visible after async view refreshes");
...
assert.match(production,/WORKSPACE_RUNTIME_ASSET="\/js\/workspace-runtime-20260926b\.js"/);
