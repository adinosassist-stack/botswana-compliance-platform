...
assert.match(source,/AGENT_TASK_EXECUTION_DENIED/);
assert.match(source,/task_execution_verification_failed/);
assert.match(source,/verified:true/);
assert.match(source,/agent_execution_receipts/);
assert.match(source,/async function listTaskRequests/);
assert.match(source,/ORDER BY q\.created_at DESC/);
assert.match(source,/async function cancelTask/);
assert.match(source,/AGENT_TASK_CANCELLED/);
assert.match(source,/status IN \('prepared','approved'\)/);
assert.ok(source.includes('if(path==="/api/agentic/task-execution/requests"&&request.method==="GET")return listTaskRequests(env,auth);'));
assert.ok(source.includes('const cancel=path.match(/^\\/api\\/agentic\\/task-execution\\/requests\\/([^/]+)\\/cancel$/);'));
assert.match(source,/activeGrants/);
assert.match(source,/JOIN agent_delegations d ON d\.id=g\.delegation_id/);
...
assert.ok(entry.includes(`const V81_SCHEMA_DELTA="${releaseProfile.latest_cloudflare_migration}"`),"bounded-task entry must follow the authoritative reviewed schema tip");
