...
assert.match(migrationScript,/function splitMigrationStatements\(sql\)/);
assert.match(migrationScript,/\^CREATE\\s\+TRIGGER\\b/);
assert.match(migrationScript,/\^END;\\s\*\$/);
assert.match(migrationScript,/statements\.length!==13/);
assert.match(migrationScript,/batch:statements\.map\(sql=>\(\{sql,params:\[\]\}\)\)/);
assert.match(migrationScript,/await batch\(statements\)/);
