...
ok(ownerBrief.includes('currentCash+projectedMonthlyRevenue-outflows-(purchase||0)')&&ownerBrief.includes('recommendedCashImpact=purchaseDelayImpact+(salesCashImpact||0)')&&ownerBrief.includes('monthEndAfter=monthEndBefore!==null?monthEndBefore+recommendedCashImpact:null'), 'cash simulation exposes deterministic purchase and sales scenario impact');
ok(ownerBrief.includes('canView=()=>["owner","manager"].includes(role())')&&ownerBrief.includes('canEdit=()=>role()==="owner"')&&ownerBrief.includes('canEditSales=()=>["owner","manager"].includes(role())'), 'owner command centre preserves management visibility, owner-only financial assumptions and owner-manager sales editing');
ok(ownerBrief.includes('request("/api/state",{method:"PUT"')&&ownerBrief.includes('version:latestStateEnvelope.version'), 'owner assumptions and sales records persist through the versioned tenant state boundary');
...
ok(ownerBrief.includes('const MAX_OPPORTUNITIES=500')&&ownerBrief.includes('const MAX_CAMPAIGNS=50'), 'sales layer has bounded per-company record limits');
...
ok(ownerBrief.includes('function conversionForPeriod')&&ownerBrief.includes('row.status==="won"||row.status==="lost"')&&ownerBrief.includes('resolved.length?won.length/resolved.length*100:null'), 'conversion uses only resolved won/lost quotation cohorts and excludes open quotes');
ok(ownerBrief.includes('currentStart=addDays(date,-(windowDays-1))')&&ownerBrief.includes('previousStart=addDays(previousEnd,-(windowDays-1))')&&ownerBrief.includes('cur.resolved>=3&&prev.resolved>=3'), 'location conversion comparison uses consecutive 30-day cohorts and minimum resolved evidence');
ok(ownerBrief.includes('dormantDays')&&ownerBrief.includes('daysSince(row.lastContactAt||row.quotedAt,date)')&&ownerBrief.includes('dormantValue'), 'dormant quotation detection is based on recorded follow-up age and quote value');
...
ok(ownerBrief.includes('best.roas>=worst.roas*1.25')&&ownerBrief.includes('Math.min(worst.monthlySpendBwp*.25,worst.monthlySpendBwp)')&&ownerBrief.includes('incrementalRevenue:move*(best.roas-worst.roas)'), 'campaign reallocation requires material ROAS separation and models a bounded 25% shift');
...
ok(ownerBrief.includes('same-month collection')&&ownerBrief.includes('salesRevenueScenario')&&ownerBrief.includes('salesCashImpact'), 'sales-to-cash simulation requires an explicit collection assumption');
...
ok(!projectedState.includes('salesIntelligence')&&ownerBrief.includes('if(!canView()){shell.hidden=true;return}'), 'reviewer/auditor state projection and owner brief visibility do not expose sales records');
