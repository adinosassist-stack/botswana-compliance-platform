...
ok(worker.includes('/api/evidence/presign')&&worker.includes('/api/evidence/integrity-upload')&&worker.includes('/api/evidence/upload'), 'all direct evidence upload entry routes are covered by the production cap');
ok(worker.includes(String.raw`/^\/api\/evidence\/[^/]+\/upload$/`), 'presigned evidence PUT route is covered by the production cap');
ok((worker.match(/readBytesBounded\(req,\{maxBytes:EVIDENCE_MAX_BYTES\}\)/g)||[]).length>=2, 'production upload routes use bounded streaming body readers');
