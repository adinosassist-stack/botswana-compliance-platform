...
assert.ok(html.includes('(employees.items||[]).filter(x=>String(x.status||\"\").trim().toLowerCase()===\"active\"&&!recentlyRemovedEmployeeIds.has(String(x.id)))'),'reporting selector must exclude inactive and freshly removed employees while accepting legacy active casing');
...
assert.ok(worker.includes('/^\\/api\\/daily-reporting\\/locations\\/[^/]+(?:\\/(?:deactivate|reactivate))?$/.test(url.pathname)')&&worker.includes('url.pathname==="/api/daily-reporting/access"')&&worker.includes('url.pathname==="/api/daily-reporting/dashboard"')&&worker.includes('const featureKey=reportingSetupPath?"operating_locations":"daily_operations"'),'location lifecycle, reporting access and the basic dashboard must use operating_locations while advanced analytics remain daily_operations-gated');
