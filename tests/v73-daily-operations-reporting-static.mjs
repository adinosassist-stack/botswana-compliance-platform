...
assert.ok(worker.includes('const reportingSetupPath=url.pathname==="/api/daily-reporting/locations"||url.pathname==="/api/daily-reporting/access"')&&worker.includes('const featureKey=reportingSetupPath?"operating_locations":"daily_operations"'),'location and reporting-access setup must use the operating_locations entitlement while analytics remain daily_operations-gated');
