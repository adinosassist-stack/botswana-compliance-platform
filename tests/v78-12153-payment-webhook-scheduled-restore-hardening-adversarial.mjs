...
const latestCloudflareMigrationNumber=Number(String(profile.latest_cloudflare_migration||"").slice(0,3));ok(Number.isInteger(latestCloudflareMigrationNumber)&&latestCloudflareMigrationNumber>=38&&(worker.includes(`const EXPECTED_SCHEMA_DELTA="${profile.latest_cloudflare_migration}"`)||agenticEntry.includes(`const V81_SCHEMA_DELTA="${profile.latest_cloudflare_migration}"`)),"release and readiness preserve migration 038 or reviewed successor");
