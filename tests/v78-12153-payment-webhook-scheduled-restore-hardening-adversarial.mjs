...
const migrationTip=String(profile.latest_cloudflare_migration||""),migrationTipNumber=Number(migrationTip.slice(0,3));ok(Number.isInteger(migrationTipNumber)&&migrationTipNumber>=38&&(worker.includes(`const EXPECTED_SCHEMA_DELTA="${migrationTipNumber>=47?"046_v80_agentic_outcomes.sql":migrationTip}"`)||agenticEntry.includes(`const V81_SCHEMA_DELTA="${migrationTip}"`)),"release and readiness preserve migration 038 or reviewed successor")
