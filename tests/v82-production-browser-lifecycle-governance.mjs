...
must(wrapper,/async function probeAppScriptIsolation\(browser,cookies,label,blockedPaths\)/,'browser proof has an isolated app-script A/B diagnostic');
must(wrapper,/blocked\.has\(url\.pathname\)[\s\S]*route\.abort\('blockedbyclient'\)/,'app-script diagnostic blocks only explicit same-origin asset paths');
must(wrapper,/\['\/js\/surface-boundaries\.js'\]/,'app-script diagnostic isolates the workspace boundary runtime');
must(wrapper,/\['\/js\/oauth-availability\.js'\]/,'app-script diagnostic isolates the OAuth availability runtime');
must(wrapper,/\['\/js\/surface-boundaries\.js','\/js\/oauth-availability\.js'\]/,'app-script diagnostic isolates both app-only runtimes together');
must(wrapper,/desktop app-script isolation \$\{label\} state[\s\S]*10000/,'app-script state observation has an external Node deadline');
must(wrapper,/desktop app-script isolation \$\{label\} close[\s\S]*BROWSER_CLOSE_DEADLINE_MS/,'app-script diagnostic teardown is externally bounded');
mustNot(wrapper,/desktop app-script isolation[^\n]*(cookie|password|csrf|token)/i,'app-script diagnostic logs no credentials or session secrets');
...
const appScriptIsolationIndex=wrapper.indexOf("activeStage='desktop app-script isolation diagnostics'");
const desktopAppNavigationIndex=wrapper.indexOf("activeStage='desktop app navigation'");
assert.ok(browserLoginMarkIndex>=0&&authStateProbeIndex>browserLoginMarkIndex&&appScriptIsolationIndex>authStateProbeIndex&&desktopAppNavigationIndex>appScriptIsolationIndex,'FAIL auth-surface state and app-script isolation diagnostics must run after browser login and before canonical app navigation');
console.log('PASS auth-surface state and app-script isolation diagnostics run before canonical workspace document load');
...
must(agenticEntry,/audit_start/,'synthetic trace marks audit hydration start');
must(agenticEntry,/audit_complete/,'synthetic trace marks audit hydration resolution');
...
must(agenticEntry,/x-thebe-synthetic-boot-trace","auth-state-render-v2/,'synthetic traced documents carry an explicit non-secret render diagnostic response marker');
