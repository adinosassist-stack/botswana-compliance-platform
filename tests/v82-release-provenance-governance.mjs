...
const checks = [
  ...
  ['prior release baseline must have successful automatic post-deploy smoke', bf07.includes('/actions/workflows/postdeploy-smoke.yml/runs?branch=main&per_page=100') && bf07.includes('nearest prior [deploy] marker has no successful automatic post-deploy smoke on main') && bf07.includes('run.get("event")=="workflow_run"')],
  ['prior release baseline must have successful automatic Phase 0 launch audit', bf07.includes('/actions/workflows/production-launch-audit.yml/runs?branch=main&per_page=100') && bf07.includes('nearest prior [deploy] marker has no successful automatic Phase 0 launch audit on main') && bf07.includes('Prior Phase 0 launch-audit release baseline confirmed')],
