...
const checks = [
  ...
  ['automatic release authority is manifest-scoped and independent of merge-message formatting', bf07.includes("github.event_name == 'workflow_dispatch' ||\\n      github.event_name == 'push'") && !bf07.includes("startsWith(github.event.head_commit.message, '[release]')") && !bf07.includes('automatic release merge message must begin with [release]') && !bf07.includes('release_message="$(git log -1 --format=%B "$GITHUB_SHA")"')],
  ...
  ['release merge must have successful Recovery CI', bf07.includes('/actions/workflows/recovery-ci.yml/runs?branch=main&per_page=100') && bf07.includes('release merge has no successful completed Recovery CI run on main')],
  ...
  ['first-parent lineage review window is bounded and capped', bf07.includes("MAX_RELEASE_LINEAGE_MERGES: '100'") && bf07.includes('MAX_RELEASE_LINEAGE_MERGES <= 100') && bf07.includes('release lineage exceeds the bounded ${MAX_RELEASE_LINEAGE_MERGES}-merge review window') && bf07.includes('${#lineage_shas[@]} <= MAX_RELEASE_LINEAGE_MERGES')],
