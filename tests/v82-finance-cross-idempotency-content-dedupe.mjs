...
assert.notDeepEqual(__financeTest.importContentFingerprintBasis(base),__financeTest.importContentFingerprintBasis({...base,accountId:"a2"}),"account scope must be part of content identity");
assert.notDeepEqual(__financeTest.importContentFingerprintBasis(base),__financeTest.importContentFingerprintBasis({...base,sourceType:"manual"}),"source type must be part of content identity");
assert.notDeepEqual(__financeTest.importContentFingerprintBasis({...base,rows:[{...rows[0],sourceId:"source-1"}]}),__financeTest.importContentFingerprintBasis({...base,rows:[{...rows[0],sourceId:"source-2"}]}),"explicit source ids must remain significant");
