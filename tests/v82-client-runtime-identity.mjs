...
const request=new Request('https://thebedesk.com/?runtime-test=1');
...
assert.equal(decorated.headers.get('x-thebe-client-release'),clientRuntimeRelease(),'HTML response must expose runtime identity');
assert.equal(decorated.headers.get('x-thebe-client-implementation'),CLIENT_TRANSPORT_IMPLEMENTATION,'HTML response must expose implementation identity');
assert.match(decorated.headers.get('cache-control')||'',/no-store/,'HTML response must be non-cacheable');
assert.equal(decorated.headers.get('etag'),null,'runtime-versioned HTML must not retain stale ETag');
assert.ok((await decorated.text()).includes(`/js/api-client.js?v=${clientRuntimeRelease()}`),'HTML response must load the exact runtime identity');
