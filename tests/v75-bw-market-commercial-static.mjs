...
const checks=[
 ...
 ["daily ops entitlement",w.includes('url.pathname.startsWith("/api/daily-reporting/")')&&w.includes('const reportingSetupPath=url.pathname==="/api/daily-reporting/locations"||url.pathname==="/api/daily-reporting/access"')&&w.includes('const featureKey=reportingSetupPath?"operating_locations":"daily_operations"')&&w.includes("requireEntitlement(env,a.tenant_id,featureKey)")],
