...
try{
  ...
  ok(/^password-reset\/[a-f0-9]{64}$/.test(captured?.options?.headers?.['idempotency-key']||''),'Resend idempotency uses a token digest');
  ok(!String(captured?.options?.headers?.['idempotency-key']||'').includes('a b&c'),'raw reset token is not exposed in the idempotency header');
