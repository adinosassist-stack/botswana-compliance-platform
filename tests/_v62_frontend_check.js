...
async function renderPassport(){
  ...
  try{
    const r=await apiJson("/api/passport");
    ...
    document.getElementById("passportShares").textContent=(r.shares||[]).filter(x=>!x.revoked_at).length;
    list.innerHTML=(r.controls||[]).map(x=>`<div class="item"><b>${escapeHtml(x.control_key)}</b><div class="muted small">${escapeHtml(x.status)}${x.expires_at?` · expires ${new Date(x.expires_at).toLocaleDateString()}`:""}</div></div>`).join("")||'<div class="muted small">No controls verified yet.</div>';
    document.getElementById("passportShareList").innerHTML=(r.shares||[]).map(x=>`<div class="item"><b>${escapeHtml(x.label||"Passport share")}</b><div class="muted small">${x.revoked_at?"Revoked":"Active"}${x.expires_at?` · expires ${new Date(x.expires_at).toLocaleDateString()}`:""}</div></div>`).join("");
  }catch(e){}
}
async function createPassportShare(){
  try{
    const r=await apiJson("/api/passport/share",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({label:"Shared compliance passport"})});
    alert(`Share token created. Copy it now: ${r.shareToken}`);
...
async function renderPassportShares(){
  ...
  try{
    ...
    box.innerHTML=(r.items||[]).map(x=>`<div class="item"><div class="between row"><div><b>${escapeHtml(x.label||"Compliance Passport")}</b><div class="muted small">${x.revoked_at?"revoked":(x.expires_at&&new Date(x.expires_at)<new Date()?"expired":"active")} · ${x.view_count||0}${x.max_views!=null?`/${x.max_views}`:""} views · expires ${x.expires_at?new Date(x.expires_at).toLocaleString():"never"}</div></div>${!x.revoked_at?`<button class="btn alt" onclick="revokePassportShare('${x.id}')">Revoke</button>`:""}</div></div>`).join("")||'<div class="muted small">No active shares.</div>';
  }catch(e){}
}
...
async function bootstrap(){marketingGate.classList.add("hidden");authGate.classList.add("hidden");appShell.style.visibility="hidden";if(STANDALONE_PREVIEW){showMarketing();return}try{await loadServerState()}catch(err){csrfToken="";currentUser=null;showMarketing()}}
