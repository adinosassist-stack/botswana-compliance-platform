...
assert.match(runner,/execFile\(process\.execPath,args/);
assert.match(runner,/d1','execute','DB','--remote'/);
assert.match(runner,/--file=\$\{spec\.path\}/);
assert.match(runner,/--config=\$\{wranglerConfig\}/);
assert.match(runner,/WRANGLER_JS is empty/);
assert.match(runner,/WRANGLER_CONFIG is empty/);
assert.doesNotMatch(runner,/extractReviewedStatements|splitSqliteMigrationStatements/);
...
assert.match(workflow,/Prepare pinned Wrangler D1 migration transport/);
assert.match(workflow,/binding = "DB"/);
assert.match(workflow,/database_name = "bw-compliance-os"/);
assert.match(workflow,/WRANGLER_JS=%s/);
assert.match(workflow,/WRANGLER_CONFIG=%s/);
...
console.log("v156 production migrations 051-056 Wrangler transport and rollback contract passed");
