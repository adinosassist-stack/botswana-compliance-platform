...
assert.match(loop,/if\(!outcome\.persisted\)\{[\s\S]*outcome\?\.recovery\?\.decision==="owner_attention"[\s\S]*await escalateOwnerAttention\(env,task,claim,outcome\)[\s\S]*await failObservationClaim\(env,task,claim,outcome\)/);
