...
ok('outer release authority claims registration proof once and hands server-only verification state downstream',releaseGovernanceEntry.includes('const proof=await validateAndClaimRegistrationProof(request,env,parsed.body?.turnstileToken)')&&releaseGovernanceEntry.includes('__THEBE_REGISTRATION_PROOF_VERIFIED:true')&&releaseGovernanceEntry.includes('APP_ENV:"registration-proof-verified"')&&releaseGovernanceEntry.includes('base.fetch(request,downstreamEnv,ctx)'));
ok('production registration wrapper skips duplicate proof consumption only for the server-side handoff and remains fail closed when invoked directly',productionEntry.includes('if(env?.__THEBE_REGISTRATION_PROOF_VERIFIED===true)')&&productionEntry.includes('const proof=await verifyRegistrationProof(request,env,body?.turnstileToken)')&&productionEntry.includes('return worker.fetch(request,verifiedEnv,ctx)'));
ok('registration challenge durable budget is consumed once across the outer and inner production entries',releaseGovernanceEntry.includes('durableRegistrationChallengeGate(request,env)')&&releaseGovernanceEntry.includes('__THEBE_REGISTRATION_CHALLENGE_BUDGET_VERIFIED:true')&&productionEntry.includes('if(env?.__THEBE_REGISTRATION_CHALLENGE_BUDGET_VERIFIED!==true)')&&productionEntry.includes('durableRegistrationChallengeGate(request,env)'));
ok('registration proof production wrapper still imports the durable registration boundary for direct-entry fail-closed fallback',productionEntry.includes('from "./registration-boundary.js"')&&productionEntry.includes('const verifyRegistrationProof=validateAndClaimRegistrationProof;'));
