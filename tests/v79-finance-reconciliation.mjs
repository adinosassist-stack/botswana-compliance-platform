...
assert.match(launch,/through `050_v115_manual_bank_subscriptions\.sql`/);
assert.doesNotMatch(launch,/No new schema migration is required\./);
assert.match(deploy,/Current reviewed schema delta: 050_v115_manual_bank_subscriptions\.sql/);
assert.match(cloudflareReadme,/`migrations\/050_v115_manual_bank_subscriptions\.sql`/);
for(const path of [
