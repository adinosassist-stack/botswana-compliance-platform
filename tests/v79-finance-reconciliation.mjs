...
assert.equal(profile.latest_cloudflare_migration,"056_v154_agent_control_plane.sql");
...
assert.match(launch,/through `056_v154_agent_control_plane\.sql`/);
assert.doesNotMatch(launch,/No new schema migration is required\./);
assert.match(deploy,/Current reviewed schema delta: 056_v154_agent_control_plane\.sql/);
assert.match(cloudflareReadme,/056_v154_agent_control_plane\.sql/);
for(const path of [
