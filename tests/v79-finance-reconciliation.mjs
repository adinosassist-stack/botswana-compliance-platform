...
assert.equal(profile.latest_cloudflare_migration,"055_v149_finance_watch_audit_integrity.sql");
...
assert.match(launch,/through `055_v149_finance_watch_audit_integrity\.sql`/);
assert.doesNotMatch(launch,/No new schema migration is required\./);
assert.match(deploy,/Current reviewed schema delta: 055_v149_finance_watch_audit_integrity\.sql/);
assert.match(cloudflareReadme,/`migrations\/055_v149_finance_watch_audit_integrity\.sql`/);
for(const path of [
