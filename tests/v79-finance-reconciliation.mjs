...
assert.match(receivablesMigration,/CREATE TABLE IF NOT EXISTS finance_invoices\(/,"finance receivables migration 049 must remain packaged");
...
assert.match(finance,/expectedSnapshotHash=text\(body\.expectedSnapshotHash,64\)\.toLowerCase\(\)/);
assert.match(finance,/invalid_expected_snapshot_hash/);
assert.match(finance,/finance_reconciliation_snapshot_changed/);
assert.match(finance,/FINANCE_RECONCILIATION_STALE_APPROVAL/);
assert.match(finance,/execution:\{performed:false\}/);
assert.match(finance,/expectedSnapshotHash!==prepared\.snapshotHash/);
...
assert.equal(profile.latest_cloudflare_migration,"055_v151_finance_watch_scheduler_isolation.sql");
...
assert.match(launch,/through `055_v151_finance_watch_scheduler_isolation\.sql`/);
assert.doesNotMatch(launch,/No new schema migration is required\./);
assert.match(deploy,/Current reviewed schema delta: 055_v151_finance_watch_scheduler_isolation\.sql/);
assert.match(cloudflareReadme,/`migrations\/055_v151_finance_watch_scheduler_isolation\.sql`/);
for(const path of [
...
]) assert.match(fs.readFileSync(path,"utf8"),/migrationTipNumber/);
...
assert.match(owner,/\/api\/agentic\/finance\/reconciliation\/prepare/);
assert.match(owner,/expectedSnapshotHash:proposal\.snapshotHash/);
assert.match(owner,/Record reviewed reconciliation/);
assert.match(owner,/Owner approval is required to record this reconciliation/);
assert.match(owner,/financeReconciliationPayloadFromFields/);
