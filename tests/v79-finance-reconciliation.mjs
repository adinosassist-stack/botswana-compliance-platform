...
assert.match(launch,/through `053_v132_agent_observation_claims\.sql`/);
assert.doesNotMatch(launch,/No new schema migration is required\./);
assert.match(deploy,/Current reviewed schema delta: 053_v132_agent_observation_claims\.sql/);
assert.match(cloudflareReadme,/`migrations\/053_v132_agent_observation_claims\.sql`/);
for(const path of [
