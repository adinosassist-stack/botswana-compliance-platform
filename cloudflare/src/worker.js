...
async function versionRuntimeResponse(response,request,url){
  ...
  const immutableWorkspaceStyles=/^\/assets\/workspace-inline-[a-z0-9.-]+\.css$/i.test(url.pathname);
  if(immutableWorkspaceRuntime&&["GET","HEAD"].includes(request.method)){headers.set("cache-control","public, max-age=31536000, immutable");headers.set("cdn-cache-control","public, max-age=31536000, immutable");headers.set("x-thebe-client-release",CLIENT_RUNTIME_RELEASE);return new Response(request.method==="HEAD"?null:response.body,{status:response.status,statusText:response.statusText,headers})}
  if(immutableWorkspaceStyles&&["GET","HEAD"].includes(request.method)){headers.set("cache-control","public, max-age=31536000, immutable");headers.set("cdn-cache-control","public, max-age=31536000, immutable");headers.set("x-thebe-client-release",CLIENT_RUNTIME_RELEASE);return new Response(request.method==="HEAD"?null:response.body,{status:response.status,statusText:response.statusText,headers})}
