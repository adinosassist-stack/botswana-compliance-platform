...
export function isFinanceWatchToolList(value){
  return Array.isArray(value)&&value.length>0&&value.length<=FINANCE_WATCH_READ_ACTIONS.length&&new Set(value).size===value.length&&value.every(actionKey=>typeof actionKey==="string"&&FINANCE_WATCH_READ_SET.has(actionKey));
}
...
export function buildFinanceWatchDueQuery(limit=25){
  ...
  return Object.freeze({
    sql:`SELECT id,tenant_id,status,objective,trigger_spec_json,allowed_tools_json,budget_json,next_run_at
      ...
      WHERE status='active' AND trigger_kind='scheduled' AND next_run_at IS NOT NULL AND next_run_at<=strftime('%Y-%m-%dT%H:%M:%fZ','now')
        AND json_array_length(${safeToolsJson}) BETWEEN 1 AND ${FINANCE_WATCH_READ_ACTIONS.length}
        AND (SELECT COUNT(DISTINCT value) FROM json_each(${safeToolsJson}))=json_array_length(${safeToolsJson})
        AND NOT EXISTS (
