...
function scanRequestSignatureBase({evidenceId,claimedSha,claimedSize,timestamp,requestId,mime}){return `scan:v2:${evidenceId}:${claimedSha}:${claimedSize}:${timestamp}:${requestId}:${mime}`}
function scanTimestampFresh(value,now=Date.now()){const n=Number(value);return Number.isInteger(n)&&n>0&&Math.abs(Number(now)-n)<=MAX_SIGNATURE_AGE_MS}
...
export default {async fetch(req,env){
  ...
  const claimedSizeText=String(req.headers.get("x-evidence-size")||""),timestamp=String(req.headers.get("x-evidence-timestamp")||""),requestId=String(req.headers.get("x-evidence-request-id")||"").toLowerCase();
  const claimedSize=Number(claimedSizeText),max=Math.min(DEFAULT_MAX,Math.max(1,Number(env.MAX_SCAN_BYTES)||DEFAULT_MAX));
  if(!/^[a-f0-9-]{32,36}$/i.test(evidenceId)||!/^[a-f0-9]{64}$/.test(claimedSha)||!/^[a-f0-9]{64}$/.test(claimSig)||!/^[1-9][0-9]{0,7}$/.test(claimedSizeText)||!Number.isSafeInteger(claimedSize)||claimedSize<1||claimedSize>max||!/^[a-f0-9-]{36}$/.test(requestId))return signedJson(env,{evidenceId,sha256:claimedSha,verdict:"error",provider:"thebe-cloudmersive",error:"invalid_scan_envelope"},400);
  ...
  if(!scanTimestampFresh(timestamp))return signedJson(env,{evidenceId,sha256:claimedSha,verdict:"error",provider:"thebe-cloudmersive",error:"signature_expired"},401);
  const expectedSig=await hmacHex(env.EVIDENCE_SCAN_SECRET,scanRequestSignatureBase({evidenceId,claimedSha,claimedSize:claimedSizeText,timestamp,requestId,mime}));
  if(!safeEq(expectedSig,claimSig))return signedJson(env,{evidenceId,sha256:claimedSha,verdict:"error",provider:"thebe-cloudmersive",error:"signature_invalid"},401);
  const declared=Number(req.headers.get("content-length")||0);if(declared&&declared!==claimedSize)return signedJson(env,{evidenceId,sha256:claimedSha,verdict:"error",provider:"thebe-cloudmersive",error:"size_mismatch"},409);
  let body;
  ...
  if(body.byteLength!==claimedSize)return signedJson(env,{evidenceId,sha256:claimedSha,verdict:"error",provider:"thebe-cloudmersive",error:"size_mismatch"},409);
...
export const __scannerTest=Object.freeze({ALLOWED_MIME,DEFAULT_MAX,MAX_SIGNATURE_AGE_MS,fileNameForMime,safeEq,scanRequestSignatureBase,scanTimestampFresh});
